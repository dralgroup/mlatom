...
class columbus_methods(method_model):
    ...
    Arguments:
        command_line_arguments (list): command-line arguments passed to COLUMBUS, e.g. ['-m', '1700'], default ``'None'``
        save_files_in_current_directory (bool): whether to keep input and output files in the current directory, default ``'False'``
        working_directory (str): directory in which ``$COLUMBUS/runc`` is executed, default ``'None'``
        directory_with_input_files (str): directory containing the input files, default ``'None'``
        look_for_mocoef (bool): if ``'True'``, copies ``mocoef_mc.sp`` from ``MOCOEFS`` into ``directory_with_input_files`` after ``predict()``, default ``'False'``

    .. note::

        Two options are available: CASSCF and MRCI.
    '''
    bin_env_name = 'COLUMBUS'
    
    def __init__(self, command_line_arguments=None, save_files_in_current_directory=True, working_directory=None, directory_with_input_files='', look_for_mocoef=False, **kwargs):
        ...
        self.look_for_mocoef = look_for_mocoef
    ...
    def predict(self, 
        ...
        for mol in molDB.molecules:
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                if self.look_for_mocoef:
                    if not os.path.exists(f'{tmpdirname}/mocoef'):
                        print(f"WARNING: look_for_mocoef was set, but there is no mocoef file in the {tmpdirname} directory.")
                ...
                if mol.columbus_successful:
                    ...
                    if calculate_nacv:
                        ...
                        for initial_state, final_state in state_comb:
                            ...
                            if not os.path.exists(nacpath):
                                nacpath = f'{tmpdirname}/GRADIENTS/cartgrd.nad.drt1.state{final_state}.drt1.state{initial_state}.sp'
                            if os.path.exists(nacpath):
                                ...
                                print(f'There is no nonadiabatic coupling file. ({nacpath})')
                        if ntriplets > 1:
                            ...
                            for initial_state, final_state in state_comb:
                                ...
                                if not os.path.exists(nacpath):
                                    nacpath = f'{tmpdirname}/GRADIENTS/cartgrd.nad.drt2.state{final_state}.drt2.state{initial_state}.sp'
                                if os.path.exists(nacpath):
                                    with open(nacpath) as ff:
                                        iatom = -1
                                        for line in ff:
                    ...
                    if calculate_soc:
                        ...
                        mol.socs = [[0.0, 0.0, 0.0] for _ in range(nsinglets*ntriplets)]
                        ...
                        if os.path.exists(runls):
                            ...
                            for isinglet_state, itriplet_state in state_comb:
                                ...
                                mol.socs[isinglet_state*ntriplets+itriplet_state] = socvec
                    
                    if self.look_for_mocoef:
                        if os.path.exists(f'{tmpdirname}/MOCOEFS/mocoef_mc.sp'):
                            shutil.copy2(f'{tmpdirname}/MOCOEFS/mocoef_mc.sp', f'{self.directory_with_input_files}/mocoef')  
