...
def vibrational_analysis(masses, hessian, mode_type='MDU'):
    ...
    print('wavenumbers, modes, fconstants, rmasses')
    print(wavenumbers, modes, fconstants, rmasses)
...
def get_gau_thermo(imol, hofmethod):
    ...
    energy = os.popen("grep 'Recovered energy=' " + gau_log + " | head -n 1 | awk '{printf $3}'").readlines()[0].strip()
    energy = float(energy)
    #print(' Total energy: %15.8f Eh' % energy)
    energy = energy * au2ev
    ZPE = os.popen("grep 'Zero-point correction=' " + gau_log + " | head -n 1 | awk '{printf $(NF-1)}'").readlines()[0].strip()
    ZPE = float(ZPE) * au2kcal
    H298 = os.popen("grep 'Sum of electronic and thermal Enthalpies=' " + gau_log + " | head -n 1 | awk '{printf $NF}'").readlines()[0].strip()
