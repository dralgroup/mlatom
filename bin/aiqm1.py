...
import os
pythonpackage = True
try: from . import data, models, stopper
except:
    import data, models, stopper
    pythonpackage = False
...
try: 
    ...
    if pythonpackage:
        ...
        stopper.stopMLatom('Please install all Python modules required for TorchANI')

class aiqm1():
    available_methods = ['AIQM1', 'AIQM1@DFT', 'AIQM1@DFT*']
    ...
    def __init__(self, method='AIQM1', qm_program=None, qm_program_kwargs={}):
        self.method = method
        self.qm_program = qm_program
        self.qm_program_kwargs = qm_program_kwargs
        modelname = method.lower().replace('*','star').replace('@','at')
        ani_nn_children = []
        for ii in range(8):
            nn_i = models.model_tree_node(name=f'{modelname}_nn{ii}', operator='predict', model=ani_nns_in_aiqm1(method=method, model_index=ii))
            ani_nn_children.append(nn_i)
        ani_nns = models.model_tree_node(name=f'{modelname}_nn', children=ani_nn_children, operator='average')
        shift = models.model_tree_node(name=f'{modelname}_atomic_energy_shift', operator='predict', model=atomic_energy_shift(method=method))
...
class ani_nns_in_aiqm1():
    ...
    def predict(self, molecular_database=None, molecule=None,
        ...
        for mol in molDB.molecules:
            ...
            xyz_coordinates = torch.tensor(np.array(mol.get_xyz_coordinates()).astype('float')).to(self.device).requires_grad_(calculate_energy_gradients or calculate_hessian)
