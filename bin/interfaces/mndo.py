...
class mndo_methods():
    ...
    available_methods = list(method_keywords.keys())
    
    def __init__(self, method='ODM2*', read_keywords_from_file='', save_files_in_current_directory=False):
        self.method = method
    ...
    def predict(self, molecular_database=None, molecule=None,
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            for mol in molDB.molecules:
                ...
                mndoargs = [self.mndobin, '<', mndoinpfilename]
                cmd = ' '.join(mndoargs)
                ...
                if self.save_files_in_current_directory:
                    with open(f'{tmpdirname}/mndo{ii}.out', 'w') as fout:
                        for readable in proc.stdout:
                            fout.writelines(readable)
