...
def predict(args):    
    ...
    energyOffset = 0
    best_model = torch.load(args.mlmodelin, map_location=torch.device('cpu'))
    ...
    with open(args.yestfile,'wb') as fy,  open(args.ygradxyzestfile,'wb') as fgrad:
        ...
        for properties in test:
            ...
            predicted_energies = np.array(predicted_energies.cpu().detach().numpy())+getOffsets(self_energies_train,species).cpu().detach().numpy().astype(np.float64)
    ...
    if args.hessianestfile:
        with open(args.hessianestfile+args.setname,'wb') as fhess:
            ...
            for properties in test:
                ...
                hessians=hessians.reshape(hessians.shape[0],-1)
                # np.savetxt(fhess, hessians.flatten(), fmt='%20.12f', delimiter=" ")
                for i in range(len(species)):
                    natom=len([a for a in species[i] if a!=-1])
                    line="%d\n\n" % natom
                    fhess.write(line.encode('utf-8'))
                    np.savetxt(fhess, hessians[i], fmt='%20.12f', delimiter=" ")
...
def ani_predict(args):
    ...
    elif args.ani2x or args.ani2xd4:
    ...
    addD4 = False
    if args.ani1xd4 or args.ani2xd4:
        addD4 = True
    ...
    for i in range(len(numbers)):
        ...
        if addD4:
            dftd4.calculate(elements[i], coordinates[i], gradientCalc=True, hessianCalc=hessianCalc)
            d4_results = dftd4.get_results()
        ...
        if args.hessianestfile:
            ...
            if addD4:
                d4_hess = torch.tensor(d4_results['hessian']).flatten()
                hessian = hessian + d4_hess
            fhess.write('%d\n\n' % nat)
