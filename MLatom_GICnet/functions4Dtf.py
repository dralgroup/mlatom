...
#    t_start=time.time()
    ...
    descr, t=tf.split(xx,[xx.shape[1]-1,1],1)
    
#    print('          cast time: ', time.time()-t_start)
#    t_start=time.time()
    with tf.GradientTape() as tape:
        tape.watch(t)
        x_new = model(tf.concat((descr,t),1)) + y_shift[:l]
        # x_new = model(xx) + y_shift[:l]
#    print('          getM time: ', time.time()-t_start)
    # jacobian=tape.jacobian(x_new,xx)
    # v=jacobian[0,:l,0,-1]
#    t_start=time.time()
    jac=tape.batch_jacobian(x_new,t)
    v=jac[:,:l,-1]/x_scale[-1]
#    print('          getV time: ', time.time()-t_start, jac.shape)
#    t_start=time.time()
    y=tf.concat([x_new[:,:l],tf.cast(v,tf.float32)],1)
#    print('          cat time: ', time.time()-t_start)
    
    # print('        diff time: ', time.time()-t_0)
    return y
...
def correctAngle(x):
    ...
    for a in range(n,3*n):
        for i in range(x.shape[0]-1):
            if x[i,a]*x[i+1,a]<0 and np.abs(x[i+1,a])>1:
                if x[i+1,a]<0:
                    xx[i+1:,a]+=2*np.pi
                else:
                    xx[i+1:,a]-=2*np.pi
